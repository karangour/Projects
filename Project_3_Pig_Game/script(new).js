...
const selectors = {
  ...
  currentScores: [
    document.querySelector(".current-score1"),
    document.querySelector(".current-score2"),
  ],
  totalScores: [
...
function newGame() {
  ...
  selectors.players[1].classList.remove("active");
}
...
function gamePlay() {
  ...
  selectors.rollButton.addEventListener("click", () => {
    ...
    const activePlayerIndex = selectors.players.findIndex((player) =>
  ...
  selectors.holdButton.addEventListener("click", () => {
    ...
    totalScore.textContent =
      parseInt(totalScore.textContent, 10) +
      parseInt(currentScore.textContent, 10);
    currentScore.textContent = 0;
    if (parseInt(totalScore.textContent) > 1) {
      // selectors.game.classList.toggle("blur", "inactive");
      // selectors.instructions.classList.toggle("hidden");
      activePlayerIndex < 0
        ? (selectors.rules.textContent = "PLAYER 1 WINS!")
        : (selectors.rules.textContent = "PLAYER 2 WINS!");
      selectors.instructions.removeEventListener("click", startGame);
      selectors.game.removeEventListener("click", startGame);
      initGame();
    } else togglePlayer;
  });
